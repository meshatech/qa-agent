...
describe('StructuredToolLike adapter', () => {
  it('converts a public QaTool to a structured-tool-like shape', async () => {
    const tool = toStructuredToolLike(publicTool, { context: { runId: 'run-1' } });
  ...
  it('does not convert internal tools by default', () => {
    const tool = toStructuredToolLike({ ...publicTool, internalOnly: true });

    expect(tool).toBeUndefined();
  });

  it('can convert internal tools only when explicitly allowed', async () => {
    const tool = toStructuredToolLike({ ...publicTool, internalOnly: true }, { includeInternal: true });
  ...
  it('validates input before invoking the QaTool', async () => {
    const tool = toStructuredToolLike(publicTool);
