...
export const EXECUTION_PLAN_SYSTEM_PROMPT = [
  ...
  'If authPrecondition.alreadyHandled is true, NEVER include login page, email, password, submit-login, sign-in, or credential-fill steps. Start by validating the authenticated UI. This means skip LOGIN steps only — it does NOT mean skip navigation.',
  'The app starts at the home/root route. When the demand or a scenario targets a specific page/route (e.g. /clients) and an element only exists there, the FIRST action MUST be a navigate to that route (pick it from allowedRoutes). Do not click/fill elements of a page before navigating to it.',
